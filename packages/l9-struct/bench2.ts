...
// Бенчмарк для Vector
function benchVector(testData: TestData, withGC = false) {
  const vec = new Vector(testData.length, RGBA);
...
// Бенчмарк для Array
function benchArray(testData: TestData, withGC = false) {
  const arr = [];
...
function benchStressVector(iterations: number, withGC = false) {
  const vec = new Vector(100, RGBA);
...
function benchStressArray(iterations: number, withGC = false) {
  const arr = [];
...
function runBenchmark() {
  ...
  const vec = new Vector(memTestSize, RGBA);
